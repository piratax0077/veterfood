...
@section('title', 'Verificación de voucher')

@section('content')
<div class="card" style="max-width:760px;margin:30px auto">
    <h1>Verificación de voucher</h1>
    ...
    <p><strong>Vigencia:</strong> {{ $voucher->valido_desde?->format('d-m-Y') ?? 'Hoy' }} al {{ $voucher->valido_hasta?->format('d-m-Y') ?? 'Sin término' }}</p>
    <p><strong>Usos:</strong> {{ $voucher->usos_realizados }} / {{ $voucher->usos_maximos }}</p>
    @if($vigente)
        <div class="alert">Voucher válido. Firma segura verificada.</div>
    @else
        <div class="alert" style="background:#fee2e2;color:#991b1b">
            ...
            @if($voucher->usos_realizados >= $voucher->usos_maximos) Uso máximo alcanzado. @endif
        </div>
    @endif
